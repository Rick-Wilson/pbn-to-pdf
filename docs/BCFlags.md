...
| Page break (0x4000000) | 🔲 Planned | Not yet implemented |
| Column break (0x2000000) | 🔲 Planned | Not yet implemented |
| All other flags | 🔲 Future | Not yet implemented |

### Implied Rules (Non-BCFlags)

| Rule | Status | Notes |
|------|--------|-------|
| Empty deal hides metadata | ✅ Implemented | Board/Dealer/Vuln/Diagram hidden when no cards |
| Only-North-visible hides compass | ✅ Implemented | Uses `[Hidden "ESW"]` tag |
| Single suit hides symbol | ✅ Implemented | Fragment deals with one suit |
...
- Hide Dealer field (0x200000)
- Hide Vulnerable field (0x400000)
- Hide Contract/Declarer (0x8000000) - Note: This appears to be a typo in some files
...
**Implementation:** `src/render/layouts/analysis.rs` lines 227-231
...
- Extra line height padding is added for proper spacing

**Implementation:** `src/render/components/hand_diagram.rs` lines 131-134 and 220-223

```rust
let only_north_visible = !hidden.north && hidden.east && hidden.south && hidden.west;
if only_north_visible {
...
**Implementation:** `src/render/components/hand_diagram.rs` lines 226-227
...
When rendering boards, check the BCFlags to determine:

1. **What to show**: Use the "Show" flags (bits 0-7) to determine which sections to render.

2. **What to hide**: Use the "Hide" flags (bits 20-28) to suppress specific header fields.

3. **Layout control**: Use bits 25-26 for page/column breaks in multi-column layouts.
...
```rust
let flags = u32::from_str_radix("60001b", 16).unwrap();

let show_play = (flags & 0x01) != 0;           // true
let show_diagram = (flags & 0x08) != 0;        // true
let show_auction = (flags & 0x10) != 0;        // true
let hide_board = (flags & 0x100000) != 0;      // true
let hide_dealer = (flags & 0x200000) != 0;     // true
let hide_vulnerable = (flags & 0x400000) != 0; // true
let page_break = (flags & 0x4000000) != 0;     // false
let column_break = (flags & 0x2000000) != 0;   // false
```
