...
use printpdf::{Color, IndirectFontRef, Mm, PdfLayerReference, Rect, Rgb};
...
impl<'a> HandDiagramRenderer<'a> {
    ...
    fn draw_debug_box(&self, x: f32, y: f32, w: f32, h: f32) {
        if !self.debug_boxes {
            return;
        }
    ...
    pub fn render_deal(&self, deal: &Deal, origin: (Mm, Mm)) -> f32 {
        let (ox, oy) = origin;
        ...
        let north_y = oy.0;
        self.draw_debug_box(north_x, north_y, north_w, hand_h);
        ...
        // West hand - left side
        let west_x = ox.0;
        self.draw_debug_box(west_x, row2_y, west_w, hand_h);
        ...
        if self.settings.show_hcp {
            self.render_hcp_box(deal, (Mm(hcp_box_x), Mm(hcp_box_y)), hcp_box_size);
        }

        // South hand - positioned next to HCP box, at same Y level
        let south_y = hcp_box_y;
        self.draw_debug_box(north_x, south_y, south_w, hand_h);
        self.render_hand_cards(&deal.south, (Mm(north_x), Mm(south_y)));
    ...
    pub fn render_hand(&self, hand: &Hand, origin: (Mm, Mm), _show_hcp: bool) {
        self.render_hand_cards(hand, origin);
    }

    /// Render hand cards only (no HCP)
    /// Origin is the top-left of the visual bounding box
    fn render_hand_cards(&self, hand: &Hand, origin: (Mm, Mm)) {
        let (ox, oy) = origin;
        let line_height = self.settings.line_height;
        ...
        for (i, suit) in Suit::all().iter().enumerate() {
            ...
            self.render_suit_line(*suit, hand.holding(*suit), (Mm(ox.0), Mm(y)));
        }
    }
    ...
    fn render_suit_line(
        ...
        self.layer.set_fill_color(Color::Rgb(super::colors::BLACK));
    ...
    fn render_hcp_box(&self, deal: &Deal, origin: (Mm, Mm), box_size: f32) {
        let (ox, oy) = origin;
        let half_box = box_size / 2.0;
        ...
        self.draw_debug_box(ox.0, oy.0, box_size, box_size);
