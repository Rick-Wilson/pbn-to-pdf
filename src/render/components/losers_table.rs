//! Losers table component for declarer plan layout
//!
...
/// Light blue for "Losers" header background
...
impl<'a> LosersTableRenderer<'a> {
    ...
    pub fn render(&self, layer: &mut LayerBuilder, origin: (Mm, Mm)) -> f32 {
        ...
        // Header row: "Count Fast and Slow Losers" with blue background
        ...
        // Header row: "Decide How to Reduce Losers" with green background
        self.render_header_row(
            layer,
            ox,
            current_y,
    ...
    fn render_techniques_row(&self, layer: &mut LayerBuilder, x: f32, y: f32) {
        let labels = ["Ruffs", "Finesse", "Pitch", "Length"];
